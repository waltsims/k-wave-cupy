function test_pass = kspaceFirstOrder3D_compare_plane_waves(plot_comparisons, plot_simulations)
% DESCRIPTION:
%     Unit test to validate 3D wave propagation with comprehensive
%     feature coverage including absorption, nonlinearity, and all source
%     types. This is the 3D version of
...
%
%         91. linear + lossless + source.uz (additive) + homogeneous
%         92. linear + lossless + source.uz (additive) + heterogeneous
%         93. linear + lossless + source.uz (additive-no-correction) + homogeneous
%         94. linear + lossless + source.uz (additive-no-correction) + heterogeneous
%         95. linear + lossless + source.uz (dirichlet) + homogeneous
%         96. linear + lossless + source.uz (dirichlet) + heterogeneous
%
% ABOUT:
%     author      - Bradley Treeby (original), Claude Code (3D adaptation)
...
% Detect if Python backend is active by checking where kspaceFirstOrder3D resolves
...
func_path = which('kspaceFirstOrder3D');
using_python_backend = contains(func_path, 'shims');

% Debug: Show which function is being used
disp(['DEBUG: kspaceFirstOrder3D resolves to: ' func_path]);
disp(['DEBUG: Using Python backend: ' mat2str(using_python_backend)]);

% Reference data file path
ref_data_file = fullfile(fileparts(mfilename('fullpath')), 'kspaceFirstOrder3D_reference_data.mat');
...
source_strength = 5e6;
source_position_x = PML_size - 2;
source_position_y = Ny / 2;
source_position_z = Nz / 2;
source_freq     = 200;
...
test_names = {...
    ...
    'linear + lossless + source.uz (additive) + homogeneous', ...
    'linear + lossless + source.uz (additive) + heterogeneous', ...
    'linear + lossless + source.uz (additive-no-correction) + homogeneous', ...
    'linear + lossless + source.uz (additive-no-correction) + heterogeneous', ...
    'linear + lossless + source.uz (dirichlet) + homogeneous', ...
    'linear + lossless + source.uz (dirichlet) + heterogeneous', ...
    };
...
uz_tests = 91:96;
uz_additive_tests = [91, 92];
uz_additive_no_correction_tests = [93, 94];
uz_dirichlet_tests = [95, 96];
...
for test_num = 1:96
    ...
    % update command line
    disp(['Running Test ' num2str(test_num) ' of 96: ' test_names{test_num}]);
    ...
    switch test_num
        ...
        case {91,92,93,94,95,96}
            ...
            % linear + lossless (source.uz scenarios)
    ...
    if any(p0_tests == test_num)
        ...
        source.p0(source_position_x, source_position_y, source_position_z) = source_strength;
        if SMOOTH_P0_SOURCE
             source.p0 = smooth(source.p0, true);
        end
    elseif any(p_tests == test_num)
        source.p_mask = zeros(Nx, Ny, Nz);
        source.p_mask(source_position_x, source_position_y, source_position_z) = 1;
    ...
    elseif any(u_tests == test_num)
        ...
        source.u_mask(source_position_x, source_position_y, source_position_z) = 1;
    ...
    elseif any(uz_tests == test_num)
        source.u_mask = zeros(Nx, Ny, Nz);
        source.u_mask(source_position_x, source_position_y, source_position_z) = 1;
        source.uz = source_signal ./ (c0 * rho0);
        if any(uz_dirichlet_tests == test_num)
            source.u_mode = 'dirichlet';
        elseif any(uz_additive_tests == test_num)
            source.u_mode = 'additive';
        elseif any(uz_additive_no_correction_tests == test_num)
            source.u_mode = 'additive-no-correction';
        else
            error('Unknown source mode.');
        end
    ...
    sensor.mask(sensor_position_x, sensor_position_y, sensor_position_z) = 1;

    % run simulation
    sensor_data_3D = kspaceFirstOrder3D(kgrid, medium, source, sensor, input_args{:}, 'PMLAlpha', PML_alpha);
    ...
    if using_python_backend
        ...
        if ref_max > 0
            rel_error = max(abs(sensor_data_3D(:) - ref_sensor_data(:))) / ref_max;
        else
            rel_error = max(abs(sensor_data_3D(:)));
        end

        % Check threshold
        COMPARISON_THRESH = 1e-12;  % 3D has more FFT ops, so fp error is higher than 1D
        ...
        reference_results{test_num} = sensor_data_3D;

        % Basic sanity check
        if isempty(sensor_data_3D) || any(isnan(sensor_data_3D(:))) || any(isinf(sensor_data_3D(:)))
...
function setMaterialProperties()
    ...
    % sound speed and density (heterogeneous step in x-direction, uniform in y and z)
    ...
    if isfield(medium, 'BonA')
        medium.BonA = BonA0 * ones(Nx, Ny, Nz);
